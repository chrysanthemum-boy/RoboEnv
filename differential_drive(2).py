...
from regulator_model_cz import RegulatorModel
...
def main():
    ...
    # Initialize state for linearization
    ...
    linearize_around_goal = False  # Set to True to use fixed goal state

    if linearize_around_goal:
        # Option 1: Linearize around goal state and action
        regulator.updateSystemMatrices(sim, goal_state, goal_action)
    else:
        # Option 2: Linearize around current state and control at each step
    ...
    while True:
        ...
        # Update matrices if using current state linearization
        if not linearize_around_goal:
    ...
    plt.plot([pos[0] for pos in base_pos_all], [pos[1] for pos in base_pos_all], label="Trajectory")
