...
from simulation_and_control import differential_drive_regulation_controller, regulation_polar_coordinates, \
    regulation_polar_coordinate_quat, wrap_angle
...
def main():
    ...
    Q = np.eye(state_dim) * 0.0005
    ## TODO update this Measurement noise covariance
    R = np.eye(meas_dim) * 0.0005
    # Observation matrix
    ...
    while True:
        ...
        if current_waypoint_index < num_waypoints:
            ...
            angular_wheels_velocity = differential_drive_controller_adjusting_bearing(base_pos, base_bearing_,
                                                                                      des_base_pos, base_bearing_d,
                                                                                      wheel_radius, wheel_base_width,
                                                                                      kp_pos, kp_ori)
    ...
    # Plotting# 设置采样间隔
    sampling_interval = 50  # 每 25 个点采样一个

    # 采样数据
    sampled_base_pos_all = base_pos_all[::sampling_interval]
    sampled_base_ori_all = base_ori_all[::sampling_interval]

    # 设置seaborn调色板
    palet = sns.color_palette("Paired",6)
    # palette = sns.cubehelix_palette(8)

    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 8))
    # 绘制散点图和箭头图
    # 绘制散点图
    ax1.scatter([pos[0] for pos in sampled_base_pos_all], [pos[1] for pos in sampled_base_pos_all],
                label='Base Position', zorder=2, color=palet[3])
    # plt.plot([pos[0] for pos in base_pos_all], [pos[1] for pos in base_pos_all], label='Base Position',color=palette[2],
    #          zorder=1)
    # 绘制箭头图
    x = [pos[0] for pos in sampled_base_pos_all]
    y = [pos[1] for pos in sampled_base_pos_all]
    u = [np.cos(wrap_angle(pin.rpy.matrixToRpy(pin.Quaternion(ori[3], ori[0], ori[1], ori[2]).toRotationMatrix())[2]))
         for ori in sampled_base_ori_all]
    v = [np.sin(wrap_angle(pin.rpy.matrixToRpy(pin.Quaternion(ori[3], ori[0], ori[1], ori[2]).toRotationMatrix())[2]))
         for ori in sampled_base_ori_all]
    ax1.quiver(x, y, u, v, angles='xy', scale_units='xy', scale=8, label='Base Orientation',alpha=0.5, zorder=3, color=palet[5])

    # 使用seaborn绘制KDE
    # cmap = sns.cubehelix_palette(light=1, as_cmap=True)
    sns.kdeplot(x=[pos[0] for pos in base_pos_all], y=[pos[1] for pos in base_pos_all], fill=True,
                bw_adjust=.5, zorder=0, ax=ax1, cmap='Blues')

    ax1.set_title('Base Orientation (noise = 0.001)')
    ax1.set_xlabel('X Position (m)')
    ax1.set_ylabel('Y Position (m)')
    ax1.legend()

    # Second subplot: Add your new plot here
    # Example: Plotting the x and y positions over time
    ax2.plot([pos[0] for pos in base_pos_all], [pos[1] for pos in base_pos_all], label='Base Position',
             zorder=1)
    ax2.set_title('Base Positions (noise = 0.001)')
    ax2.set_xlabel('X Position (m)')
    ax2.set_ylabel('Y Positionsr (m)')
    ax2.legend()

    plt.tight_layout()
    plt.savefig('base_position_orientation_0.001_true.png', dpi=300)
    plt.show()
