...
def main():
    ...
    # Choice of linearization option
    linearization_option = 2  # Set to 1 for option 1, 2 for option 2

    # Initial linearization (around origin or current state)
    if linearization_option == 1:
        state_x_for_linearization = np.zeros(num_states)  # Goal state (0, 0, 0)
        cur_u_for_linearization = np.zeros(num_controls)  # Goal action (0, 0)
    else:
        state_x_for_linearization = sim.GetBasePosition() + [quaternion2bearing(*sim.GetBaseOrientation())]
        cur_u_for_linearization = [0.0, 0.0]  # Start with zero control input

    # Update system matrices A and B for the initial condition
    ...
    while True:
        ...
        if linearization_option == 2:
            state_x_for_linearization = np.array([base_pos[0], base_pos[1], base_bearing_])
            cur_u_for_linearization = np.array([v_linear, v_angular])
            regulator.updateSystemMatrices(sim, state_x_for_linearization, cur_u_for_linearization)
