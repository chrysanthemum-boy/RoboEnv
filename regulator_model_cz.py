...
class RegulatorModel:
    ...
    def updateSystemMatrices(self,sim,cur_x,cur_u):
        ...
        x, y, theta = cur_x  # assuming cur_x = [x, y, theta]
        v, omega = cur_u  # assuming cur_u = [v, omega]

        # Linearized continuous-time A_c matrix
        Ac = np.array([
            [0, 0, -v * np.sin(theta)],
            [0, 0, v * np.cos(theta)],
            [0, 0, 0]
        ...
        # Linearized continuous-time B_c matrix
        Bc = np.array([
            [np.cos(theta), 0],
            [np.sin(theta), 0],
            [0, 1]
        ...
        # Discretize the matrices using A = I + Δt * A_c and B = Δt * B_c
        A = np.eye(num_states) + time_step * Ac
        B = time_step * Bc
