...
def getSystemMatrices(sim, num_joints, damping_coefficients=None):
    ...
    for i in range(num_joints):
        ...
        B[num_joints + i, i] = time_step

    if damping_coefficients is not None:
        for i in range(num_joints):
            A[num_joints + i, num_joints + i] -= damping_coefficients[i] * time_step
...
def main():
    ...
    return q_mes_all, qd_mes_all


def draw_image(num_joint, q_mes_all, qd_mes_all):  # Plotting
    for i in range(2):
        ...
        plt.tight_layout()
        plt.savefig(f"joint_{i + 1}.png", dpi=300)
    plt.show()
